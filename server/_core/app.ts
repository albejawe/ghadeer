...
import { registerLocalWarehouseBatchApi } from "../localWarehouseBatchApi.js";
...
export function buildApp() {
  ...
  registerLocalWarehouseBatchApi(app);
  ...
  app.get("/api/sync/run", async (req, res) => {
    try {
      ...
      const summary = await applyFullSnapshot(
        result.invoices,
        eventId,
        "manual-full-sync"
      );
      ...
      const code =
        message === "SYNC_NOT_CONFIGURED" || message === "TURSO_NOT_CONFIGURED"
          ? 503
          : 502;
  ...
  app.post("/api/sync/sheets", async (req, res) => {
    try {
      ...
      if (!token || req.body?.token !== token)
        return res.status(401).json({ ok: false, error: "UNAUTHORIZED" });
      if (!req.body || !Array.isArray(req.body.rows) || !req.body.syncToken)
        return res.status(400).json({ ok: false, error: "INVALID_SNAPSHOT" });
      if (await hasSyncEvent(String(req.body.syncToken)))
        return res.json({
          ok: true,
          accepted: true,
          ignored: true,
      ...
      const persisted = await applyFullSnapshot(
        invoices,
        String(req.body.syncToken),
        String(req.body.source || "google-sheets"),
        fullSnapshot
      );
      res.json({
        ok: true,
  ...
  app.get("/api/shared-links/public/:id", async (req, res) => {
    try {
      const link = await getSharedLink(req.params.id);
      if (!link)
        return res.status(404).json({ ok: false, error: "LINK_NOT_FOUND" });
      const invoices = await listCachedInvoices();
      const filters = link.filters as Record<string, string>;
      res.json({
        ok: true,
        link,
        invoices: applyInvoiceFilters(invoices as never[], filters),
      });
    } catch {
  ...
  app.post("/api/shared-links", async (req, res) => {
    try {
      ...
      const link = await createSharedLink(
        req.body?.filters || {},
        String(req.body?.name || "رابط تقرير")
      );
      res.status(201).json({ ok: true, link });
    } catch {
      res.status(503).json({ ok: false, error: "LINK_CREATE_FAILED" });
    }
  });
  app.patch("/api/shared-links/:id", async (req, res) => {
    try {
      ...
      res.status(503).json({ ok: false, error: "LINK_UPDATE_FAILED" });
    }
  });
  app.delete("/api/shared-links/:id", async (req, res) => {
    try {
      ...
      res.status(503).json({ ok: false, error: "LINK_DELETE_FAILED" });
    }
  });
  app.get("/api/invoices", async (_req, res) => {
    try {
      res.json({ ok: true, invoices: await listCachedInvoices() });
    } catch {
      res.status(503).json({ ok: false, error: "DATABASE_UNAVAILABLE" });
    }
  });
  app.post("/api/invoices", async (req, res) => {
    try {
      ...
      if (!invoice?.id || !invoice.company || !invoice.number)
      ...
      res.status(502).json({ ok: false, error: "INVOICE_CREATE_FAILED" });
    }
  });
  app.patch("/api/invoices/:id", async (req, res) => {
    try {
      ...
      if (!user)
        return res.status(401).json({ ok: false, error: "UNAUTHORIZED" });
      const invoice = {
        ...(req.body as import("../sync.js").SheetInvoice),
        id: req.params.id,
      };
      if (!invoice.company || !invoice.number)
        return res.status(400).json({ ok: false, error: "INVALID_INVOICE" });
      const saved = await updateInvoiceWithSync(invoice);
      res.json({ ok: true, invoice: saved });
    } catch {
      res.status(502).json({ ok: false, error: "INVOICE_UPDATE_FAILED" });
    }
  });
  app.delete("/api/invoices/:id", async (req, res) => {
    try {
      ...
      res.status(502).json({ ok: false, error: "INVOICE_DELETE_FAILED" });
    }
  });
  // tRPC API
  app.use(
