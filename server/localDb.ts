...
export async function ensureLocalSchema() {
  ...
  schemaPromise = getTursoClient()
    .batch(
      [
        {
          sql: "CREATE TABLE IF NOT EXISTS sessions (token_hash TEXT PRIMARY KEY, user_id TEXT NOT NULL, expires_at TEXT NOT NULL, created_at TEXT NOT NULL, FOREIGN KEY(user_id) REFERENCES app_users(id) ON DELETE CASCADE)",
          args: [],
        },
        {
          sql: "CREATE INDEX IF NOT EXISTS sessions_expiry_idx ON sessions(expires_at)",
          args: [],
        },
        {
          sql: "CREATE INDEX IF NOT EXISTS idx_warehouse_sale_date ON warehouse_monthly_sales(sale_date)",
          args: [],
        },
        {
          sql: "CREATE INDEX IF NOT EXISTS idx_warehouse_gov_period ON warehouse_monthly_sales(governorate_id, year, month)",
          args: [],
        },
      ],
      "write"
...
export async function getUserBySessionToken(token: string | undefined) {
  ...
  return result.rows[0]
    ? toUser(result.rows[0] as Record<string, unknown>)
    : null;
}

export async function getRequestUser(req: Request) {
  return getUserBySessionToken(
    parseCookie(req.headers.cookie || "")[LOCAL_SESSION_COOKIE]
  );
}
...
export async function createUser(input: {
  ...
  await getTursoClient().execute({
    sql: "INSERT INTO app_users (id, username, display_name, role, password_hash, governorate_id, active, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?)",
    args: [
      id,
...
export async function listUsers() {
  const result = await getTursoClient().execute(
    "SELECT id, username, display_name, role, governorate_id, active, created_at, updated_at FROM app_users ORDER BY display_name"
  );
...
export async function deactivateSupervisor(id: string) {
  const now = new Date().toISOString();
  await getTursoClient().batch(
    [
      {
        sql: "UPDATE app_users SET active = 0, updated_at = ? WHERE id = ? AND role = 'supervisor'",
        args: [now, id],
      },
