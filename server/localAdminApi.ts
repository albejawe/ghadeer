...
router.get("/warehouse-sales", async (req, res) => {
  try {
    ...
    if (req.query.governorateId) { where.push("w.governorate_id = ?"); args.push(String(req.query.governorateId)); }
...
router.post("/warehouse-sales", async (req, res) => {
  try {
    ...
    const saleDate = String(body.saleDate || new Date().toISOString().slice(0, 10)).trim();
    ...
    await getTursoClient().execute({
      ...
      args: [id, governorateId, saleDate, year, month, quantity, amount, note, user.id, now, now],
    });

    await getTursoClient().execute({
      sql: "INSERT INTO audit_logs (id, actor_id, action, entity_type, entity_id, details_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
      args: [randomUUID(), user.id, "create", "warehouse_sale", id, JSON.stringify({ quantity, amount, saleDate }), now],
    });

    return res.status(201).json({ ok: true, id, saleDate, year, month });
  } catch (err) {
    return res.status(400).json({ ok: false, error: "WAREHOUSE_SALE_SAVE_FAILED" });
  }
});
...
router.delete("/warehouse-sales/:id", async (req, res) => {
  try {
    ...
    await getTursoClient().execute({
      sql: "INSERT INTO audit_logs (id, actor_id, action, entity_type, entity_id, details_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
      args: [randomUUID(), user.id, "delete", "warehouse_sale", id, "{}", new Date().toISOString()],
    });

    return res.json({ ok: true });
  } catch {
    return res.status(400).json({ ok: false, error: "WAREHOUSE_SALE_DELETE_FAILED" });
  }
});
